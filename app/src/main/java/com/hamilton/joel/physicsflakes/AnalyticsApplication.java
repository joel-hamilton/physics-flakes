...
//-------------------------Bitmap Methods------------------------------
    ...
    public static Bitmap getHeightScaledBitmapFromRes(int resId, int reqHeight) {
        Bitmap background;
        final BitmapFactory.Options options = new BitmapFactory.Options();
        options.inJustDecodeBounds = true;
        BitmapFactory.decodeResource(context.getResources(), resId, options);
        ...
        double height = options.outHeight;
        double whRatio = width/height;
        int newWidth = (int) (reqHeight * whRatio);

        options.inSampleSize = calculateInSampleSize(options, newWidth, reqHeight);
        options.inJustDecodeBounds = false;
        return Bitmap.createScaledBitmap(BitmapFactory.decodeResource(context.getResources(), resId, options), newWidth, reqHeight, true);
    }
